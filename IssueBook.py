...
def issue():
    ...
    extractBid = "select bid from Book"
    try:
        id=cur.execute(extractBid).fetchall()
        con.commit()
        for i in id:
            allBid.append(i[0])
        ...
        if bid in allBid:
            ...
            checkAvail = "select status from Book where bid = '"+bid+"'"
            ch=cur.execute(checkAvail).fetchall()
            con.commit()
            for i in ch:
                check = i[0]
    ...
    try:
        rn=cur.execute(extractRollno).fetchall()
        con.commit()
        for i in rn:
            allRoll.append(i[0])
    ...
    try:
        eid=cur.execute(extractEmpID).fetchall()
        con.commit()
        for i in eid:
            allEmpId.append(i[0])
