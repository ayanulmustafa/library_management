...
def search():
    ...
    searchSql = "select * from Book where title Like'%"+til+"%'"
    try:
        s=cur.execute(searchSql).fetchall()
        con.commit()
        for i in s:
...
def searchBook(): 
    ...
    Canvas1.config(bg="#5F9EA0",width = newImageSizeWidth, height = newImageSizeHeight)
